...
// 文档 https://docs.mongoing.com/mongodb-crud-operations/query-documents#or-tiao-jian
...
func (s *MongoDBTestSuite) SetupSuite() {
	...
	opts := options.Client().
		ApplyURI("mongodb://root:root@192.168.2.128:27017/").
		SetMonitor(monitor)
...
func (s *MongoDBTestSuite) TestOr() {
	...
	res, err := s.col.Find(
		ctx,
		bson.D{bson.E{Key: "$or", Value: filter}},
	)
	assert.NoError(s.T(), err)
	var arts []Article
...
func (s *MongoDBTestSuite) TestAnd() {
	...
	res, err := s.col.Find(
		ctx,
		bson.D{bson.E{Key: "$and", Value: filter}},
	)
	assert.NoError(s.T(), err)
	var arts []Article
	err = res.All(ctx, &arts)
